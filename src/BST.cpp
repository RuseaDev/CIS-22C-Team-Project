...
int BST::search(const string &key) const { return _search(root, key); }
...
void BST::remove(const string &key) { root = _remove(root, key); }
...
void BST::inorder() const { _inorder(root); }
...
void BST::printIndented() const { _printIndented(root, 0); }
